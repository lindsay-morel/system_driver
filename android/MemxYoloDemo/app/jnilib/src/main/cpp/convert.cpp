...
// Convert float32[] ? BF16[] using NEON bit-shift
void convert_bf16(float *src, uint8_t *dst, int N) {
    int i = 0;
    // Vectorized: 4 floats per iteration
    for (; i + 4 <= N; i += 4) {
        // load 4 floats
        float32x4_t vf = vld1q_f32(src + i);
        // reinterpret as uint32
        uint32x4_t vu = vreinterpretq_u32_f32(vf);
        // add rounding constant
        vu = vqaddq_u32(vu, vdupq_n_u32(0x00008000));
        // shift right 16 ? narrow to uint16x4
        uint16x4_t vb = vshrn_n_u32(vu, 16);
        // store 4 BF16 values
        vst1_u16((uint16_t*)(dst + i*2), vb);
    }
    // Scalar tail
    for (; i < N; i++) {
        uint32_t v = ((uint32_t*)src)[i] + 0x00008000;
        memcpy(dst + i*2, ((uint8_t*)&v) + 2, 2);
    }
}

// Convert BF16[] ? float32[] using NEON bit-shift
void unconvert_bf16(uint8_t *src, float *dst, int N) {
    int i = 0;
    // Vectorized: 4 at a time
    for (; i + 4 <= N; i += 4) {
        uint16x4_t vb = vld1_u16((uint16_t*)(src + i*2));
        uint32x4_t vu = vshll_n_u16(vb, 16);           // shift left 16
        float32x4_t vf = vreinterpretq_f32_u32(vu);   // bit-cast
        vst1q_f32(dst + i, vf);
    }
    // Scalar tail
    for (; i < N; i++) {
        uint32_t v = 0;
        memcpy(((uint8_t*)&v) + 2, src + i*2, 2);
        dst[i] = *((float*)&v);
    }
}