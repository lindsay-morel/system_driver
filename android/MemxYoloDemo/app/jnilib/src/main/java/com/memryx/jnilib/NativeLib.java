...
public class NativeLib {
    ...
    static {
        ...
        try {
            ...
            System.out.println("Successfully loaded jnilib library.");
        } catch (UnsatisfiedLinkError e) {
            System.err.println("Failed to load jnilib library!");
    ...
    /**
     * Perform full post-processing (softmax, bbox decode, top-K, NMS) in native C++ with NEON.
     *
     * @param output        A float array of size [numFlows][dim_h * dim_w * dim_c], where each row corresponds to a flow.
     * @param dims          An int[] of length numFlows*3, encoding for each flow: {dim_h, dim_w, dim_c}.
     * @param numFlows      The number of output flows (should be twice the number of feature levels).
     * @param confThreshold Confidence threshold: boxes with score < this will be discarded.
     * @param iouThreshold  IoU threshold for non-maximum suppression.
     * @param topK          Maximum number of boxes to keep after NMS.
     * @param inputW       Input width of the model (for scaling).
     * @param inputH       Input height of the model (for scaling).
     * @param preMax       Maximum number of boxes to consider before NMS (pre-NMS).
     * @return              An array of Detection objects representing the final post-processed results.
     */
    public native Detection[] postProcessNative(float[][] outputs, int[] dims, int numFlows, float confThreshold, float iouThreshold, int topK, int inputW, int inputH, int preMax);
}