...
static irqreturn_t memx_single_isr_handler(s32 irq, void *data)
{
	if (data) {
		...
		if (memx_dev) {
			...
			/* Wait fw complete cleared the legacy interrupt */
...
s32 memx_init_msix_irq(struct memx_pcie_dev *memx_dev)
{
	...
	s32 irq = 0;
	u32 min_msi_vec = MEMRYX_MSI_ALLOW_MIN;
	...
	memx_dev->use_legacy_irq = false;

	if (memx_dev->msix) {
		irq_request_or_err = pci_alloc_irq_vectors(memx_dev->pDev, 1,
							   MEMRYX_MAX_MSIX_NUMBER,
							   PCI_IRQ_MSIX);
		...
		if (irq_request_or_err < MEMRYX_MSI_ALLOW_MIN) {
			...
			pr_info("memryx: msix interrupt number %d not enough, try alloc msi interrupt (min=%u max=%u)\n",
				irq_request_or_err, min_msi_vec, MEMRYX_MAX_MSI_NUMBER);

			irq_request_or_err = pci_alloc_irq_vectors(memx_dev->pDev,
								   min_msi_vec,
								   MEMRYX_MAX_MSI_NUMBER,
								   PCI_IRQ_MSI);

			if (irq_request_or_err < 0) {
				pr_info("memryx: init_msi/msix irq: pci_alloc_irq_vectors(msi=%d) try using legacy interrupt\n",
					irq_request_or_err);

#if KERNEL_VERSION(6, 10, 0) > _LINUX_VERSION_CODE_
				irq_request_or_err = pci_alloc_irq_vectors(memx_dev->pDev, 1, 1,
									   PCI_IRQ_LEGACY);
#else
				irq_request_or_err = pci_alloc_irq_vectors(memx_dev->pDev, 1, 1,
									   PCI_IRQ_INTX);
#endif
				if (irq_request_or_err <= 0) {
					pr_err("memryx: init pcie legacy irq: fail to call pci_alloc_irq_vectors(%d)\n",
					       irq_request_or_err);
					return -1;
				}

				memx_dev->use_legacy_irq = true;
		...
		if (irq_request_or_err < 0) {
			pr_info("memryx: init_msi irq: pci_alloc_irq_vectors(%d) try using legacy interrupt\n",
				irq_request_or_err);
...
#ifdef DEBUG
	pr_info("memryx: requesting irq vector count %d, legacy=%d\n",
	...
	for (idx = 0; idx < memx_dev->int_info.curr_used_msix_count; idx++) {
		irq = pci_irq_vector(memx_dev->pDev, idx);
		if (irq < 0) {
			pr_err("memryx: init_msix_irq: pci_irq_vector(%u) failed (%d)\n", idx, irq);
			ret = irq;
			goto err_free_vectors;
		}

		if (memx_dev->use_legacy_irq) {
			ret = devm_request_irq(&memx_dev->pDev->dev,
					       irq,
		...
		if (ret < 0) {
			pr_err("memryx: init_msix_irq: fail to call devm_request_irq(idx=%u irq=%d ret=%d)\n",
			       idx, irq, ret);
			goto err_free_vectors;
		}
...
#ifdef DEBUG
		pr_info("memryx: irq vector[%u] = %d, legacy=%d\n",
			idx, irq, memx_dev->use_legacy_irq);
#endif
	}
...
err_free_vectors:
	while (idx--) {
		if (memx_dev->int_info.enable[idx]) {
			devm_free_irq(&memx_dev->pDev->dev,
				      memx_dev->int_info.irq[idx],
				      memx_dev);
	...
	pci_free_irq_vectors(memx_dev->pDev);
	memx_dev->int_info.curr_used_msix_count = 0;
	memx_dev->use_legacy_irq = false;
	return ret;
}
...
static void memx_dump_msix_config_info(struct memx_pcie_dev *memx_dev)
{
	...
	if (pci_read_config_dword(memx_dev->pDev, pcie_msix_cap_offset + read_dword_offset,
				  &msix_capability_doward0) != 0) {
	...
	if (pci_read_config_dword(memx_dev->pDev, pcie_msix_cap_offset + read_dword_offset,
				  &table_offset_and_mapping_in_bar_idx) != 0) {
	...
	memx_dev->int_info.msix_table =
		(struct memx_msix_entry *)(memx_dev->bar_info[table_mapping_in_bar_idx].iobase +
					   table_offset);
	memx_dev->int_info.msix_table_phy_addr =
		memx_dev->bar_info[table_mapping_in_bar_idx].base + table_offset;

	read_dword_offset += 4;

	if (pci_read_config_dword(memx_dev->pDev, pcie_msix_cap_offset + read_dword_offset,
				  &pending_bit_array_offset_and_mapping_in_bar_idx) != 0) {
	...
	pending_bit_array_mapping_in_bar_idx =
		((pending_bit_array_offset_and_mapping_in_bar_idx & 0x00000007) >> 0);
	pending_bit_array_offset =
		((pending_bit_array_offset_and_mapping_in_bar_idx & 0xFFFFFFF8));
	...
	memx_dev->int_info.pending_table =
		(struct memx_pending_entry *)(memx_dev->bar_info[pending_bit_array_mapping_in_bar_idx].iobase +
					      pending_bit_array_offset);
	memx_dev->int_info.pba_table_phy_addr =
		memx_dev->bar_info[pending_bit_array_mapping_in_bar_idx].base + pending_bit_array_offset;
