...
} PCIE_CMD_VENDOR_0_OPTION_t;

typedef struct mpu_group {
    unsigned char input_chip_id;
    unsigned char output_chip_id;
} mpu_group_t;

typedef enum memx_pcie_bar_mode {
...
typedef struct chip_info {
    ...
    memx_pcie_bar_mode_t pcie_bar_mode;
    memx_chip_role_t roles[MAX_SUPPORT_CHIP_NUM];
    mpu_group_t groups[MAX_SUPPORT_CHIP_NUM];
    CASCADE_PLUS_INPUT_DMA_TRIGGER_TYPE_t input_dma_trigger_type[MAX_SUPPORT_CHIP_NUM];
} chip_info_t;
...
typedef struct memx_reg {
    unsigned char *buf_addr;
    unsigned int  reg_start;
    unsigned int  size;
} memx_reg_t;

typedef struct memx_bin {
...
typedef struct memx_firmware_bin {
...
typedef struct memx_flow {
	unsigned char flow_id;
} memx_flow_t;

typedef struct memx_chip_id {
	unsigned char chip_id;
} memx_chip_id_t;

typedef struct memx_driver_info {
    unsigned int CDW[16];
} memx_driver_info_t;

typedef struct memx_throughput_info {
	unsigned int stream_write_us;
	unsigned int stream_write_kb;
	unsigned int stream_read_us;
	unsigned int stream_read_kb;
} memx_throughput_info_t;

typedef struct memx_xflow_param {
    unsigned char chip_id;
...
typedef struct _transport_cq{
    unsigned int status;
    unsigned int data[CQ_DATA_LEN];
} transport_cq;

typedef struct _transport_cmd{
    union{
...
typedef enum CASCADE_PLUS_ADMINCMD_ERROR_STATUS {
...
#define MEMX_DRIVER_MPU_IN_SIZE     _IOW(MEMX_IOC_MAJOR, 1, memx_mpu_size_t)
#define MEMX_DRIVER_MPU_OUT_SIZE    _IOW(MEMX_IOC_MAJOR, 2, memx_mpu_size_t)
#define MEMX_FW_MPU_OUT_SIZE        _IOW(MEMX_IOC_MAJOR, 3, memx_mpu_size_t)
#define MEMX_DOWNLOAD_FIRMWARE      _IOW(MEMX_IOC_MAJOR, 4, memx_firmware_bin_t)
#define MEMX_DOWNLOAD_DFP           _IOW(MEMX_IOC_MAJOR, 5, memx_bin_t)
#define MEMX_RUNTIMEDWN_DFP         _IOW(MEMX_IOC_MAJOR, 6, memx_bin_t)
#define MEMX_WRITE_REG              _IOW(MEMX_IOC_MAJOR, 7, memx_reg_t)
#define MEMX_READ_REG               _IOR(MEMX_IOC_MAJOR, 8, memx_reg_t)
#define MEMX_IFMAP_FLOW             _IOW(MEMX_IOC_MAJOR, 9, memx_flow_t)
#define MEMX_ABORT_TRANSFER         _IO(MEMX_IOC_MAJOR, 10)
#define MEMX_SET_CHIP_ID            _IOW(MEMX_IOC_MAJOR, 11, memx_chip_id_t)
#define MEMX_READ_CHIP_ID           _IOR(MEMX_IOC_MAJOR, 12, memx_reg_t)
#define MEMX_GET_FWUPDATE_STATUS    _IOR(MEMX_IOC_MAJOR, 13, memx_reg_t)
#define MEMX_WAIT_FW_MSIX_ACK       _IO(MEMX_IOC_MAJOR, 14)
#define MEMX_GET_HW_INFO            _IOR(MEMX_IOC_MAJOR, 15, hw_info_t)
#define MEMX_CONFIG_MPU_GROUP       _IOWR(MEMX_IOC_MAJOR, 16, hw_info_t)
#define MEMX_INIT_WTMEM_FMAP        _IOW(MEMX_IOC_MAJOR, 17, memx_chip_id_t)
#define MEMX_RESET_DEVICE           _IO(MEMX_IOC_MAJOR, 18)
#define MEMX_GET_DEVICE_FEATURE     _IOWR(MEMX_IOC_MAJOR, 19, transport_cmd)
#define MEMX_SET_DEVICE_FEATURE     _IOWR(MEMX_IOC_MAJOR, 20, transport_cmd)
#define MEMX_SET_THROUGHPUT_INFO    _IOWR(MEMX_IOC_MAJOR, 21, memx_throughput_info_t)
#define MEMX_VENDOR_CMD             _IOWR(MEMX_IOC_MAJOR, 22, transport_cmd)
#define MEMX_DUMMY_READ             _IO(MEMX_IOC_MAJOR, 23)
#define MEMX_SET_ABORT_READ         _IO(MEMX_IOC_MAJOR, 24)
#define MEMX_ADMIN_DOWNLOAD_DFP     _IOWR(MEMX_IOC_MAJOR, 25, transport_cmd)
#define MEMX_ADMIN_COMMAND          _IOWR(MEMX_IOC_MAJOR, 26, transport_cmd)
